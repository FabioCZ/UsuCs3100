...
void cpu_done::execute() {
    ...
    if (!global::cpu_queue.empty()) {
        ...
        global::utilTotal += sim_state::get_inst().currTime - global::utilStart;
        //todo probably do some stuff with utilization here
    }
...
void sim_done::execute() {
    ...
    //output simulation results here
    ...
    process_res(std::string("utilized time: ") + std::to_string(global::utilTotal));

    process_res(std::string("utilization: ") + std::to_string((global::utilTotal /sim_state::get_inst().currTime) * 100) + std::string("%"));
