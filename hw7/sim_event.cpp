...
double global::utilStart;
double global::utilTotal;
double global::round_robin_max_time = 10;
...
exec_task::exec_task(double start_time, int num_bursts, task_type type, int task_id)
    ...
    for (int i = 1; i < num_bursts; ++i) {
        if (i % 2 == 1 && i - 1 != num_bursts) {
            bursts.push_back(std::make_shared<burst>(B_IO, sim_state::get_inst().getDur_IO(type), task_id, sim_state::get_inst().getDevice_IO()));
        } else if (i % 2 == 0) {
            bursts.push_back(std::make_shared<burst>(B_CPU, sim_state::get_inst().getDur_CPU(type), task_id));
        }
    }
...
void exec_task::execute_fifo() {
    sim_event::execute();
    sim_state::log_msg(std::string("executing task, id: ") + std::to_string(task_id) + std::string(" , currBurst:") + std::to_string(curr_burst));
    if(curr_burst < num_bursts) {
        if (bursts[curr_burst]->type == B_CPU) {
            if (sim_state::get_inst().availCpu <= sim_state::get_inst().numCpu) {
                ...
                std::shared_ptr<cpu_done> a = std::make_shared<cpu_done>(sim_state::get_inst().currTime + bursts[curr_burst]->duration + sim_state::get_inst().costContext, task_id);
            ...
            if (sim_state::get_inst().availIO[bursts[curr_burst]->io_device]) {
                std::shared_ptr<io_done> a = std::make_shared<io_done>(sim_state::get_inst().currTime + bursts[curr_burst]->duration + sim_state::get_inst().costContext, task_id, bursts[curr_burst]->io_device);
                global::events.push(a);
    ...
    if(curr_burst < num_bursts) {
        if (bursts[curr_burst]->type == B_CPU) {
            if (sim_state::get_inst().availCpu <= sim_state::get_inst().numCpu) {
                ...
                if(bursts[curr_burst]->duration < sim_state::get_inst().roundRobinLimit) {   //whole CPU burst is shorter than round robin max
                    a = std::make_shared<cpu_done>(sim_state::get_inst().currTime + bursts[curr_burst]->duration + sim_state::get_inst().costContext, task_id);
                } else {
                    whole_task_fit = false;
                    a = std::make_shared<cpu_done>(sim_state::get_inst().currTime + sim_state::get_inst().roundRobinLimit + sim_state::get_inst().costContext, task_id);
            ...
            if (sim_state::get_inst().availIO[bursts[curr_burst]->io_device]) {
                ...
                if(bursts[curr_burst]->duration < sim_state::get_inst().roundRobinLimit) {
                    a = std::make_shared<io_done>(sim_state::get_inst().currTime + bursts[curr_burst]->duration + sim_state::get_inst().costContext, task_id, bursts[curr_burst]->io_device);
                } else {
                    whole_task_fit = false;
                    a = std::make_shared<io_done>(sim_state::get_inst().currTime + sim_state::get_inst().roundRobinLimit + sim_state::get_inst().costContext, task_id, bursts[curr_burst]->io_device);
                    bursts[curr_burst]->duration -= sim_state::get_inst().roundRobinLimit;
                }
                global::events.push(a);
                sim_state::get_inst().availIO[bursts[curr_burst]->io_device] = false;
...
void add_task::execute() {
    ...
    if (sim_state::get_inst().get_task_distro() <= sim_state::get_inst().taskMix) { //CPU heavy task
        ...
        sim_state::log_msg("Adding CPU heavy task, which starts at: " + std::to_string(new_start_time) + std::string(" id: ") + std::to_string(new_task->get_task_id()) + std::string(" num bursts: ") + std::to_string(new_task->num_bursts));

    } else {    //IO heavy task
        new_task = std::make_shared<exec_task>(new_start_time, sim_state::get_inst().get_burst_ct(), IO, task_id);
        sim_state::log_msg("Adding IO heavy task, which starts at: " + std::to_string(new_start_time) + std::string(" id: ") + std::to_string(new_task->get_task_id()) + std::string(" num bursts: ") + std::to_string(new_task->num_bursts));
    }
...
void cpu_done::execute() {
    ...
    if (!global::cpu_queue.empty()) {
        std::shared_ptr<cpu_done> a = std::make_shared<cpu_done>(global::cpu_queue.front()->duration + sim_state::get_inst().currTime + sim_state::get_inst().costContext, global::cpu_queue.front()->owner_task_id);
        sim_state::get_inst().availCpu--;
        global::events.push(a);
        global::cpu_queue.pop();
    } else {
        global::utilTotal += ((sim_state::get_inst().currTime - global::utilStart));// * (sim_state::get_inst().numCpu - sim_state::get_inst().availCpu - 1));
    }
...
void io_done::execute() {
    sim_event::execute();
    sim_state::log_msg(std::string("io done, io_num:") + std::to_string(io_num) + std::string(" owner task id: ") + std::to_string(owner_task_id));

    sim_state::get_inst().availIO[io_num] = true;

    if (!global::io_queue[io_num].empty()) {
        std::shared_ptr<io_done> a = std::make_shared<io_done>(global::io_queue[io_num].front()->duration + sim_state::get_inst().currTime + sim_state::get_inst().costContext, global::io_queue[io_num].front()->owner_task_id, io_num);
...
void sim_done::execute() {
    ...
    process_res(std::string("response time: min: ") + std::to_string(*std::min_element(global::response_time.begin(), global::response_time.end())));
    process_res(std::string("response time: max: ") + std::to_string(*std::max_element(global::response_time.begin(), global::response_time.end())));
    ...
    process_res(std::string("latency: min: ") + std::to_string(*std::min_element(global::latency.begin(), global::latency.end())));
    process_res(std::string("latency: max: ") + std::to_string(*std::max_element(global::latency.begin(), global::latency.end())));

    //process_res(std::string("utilized time: ") + std::to_string(global::utilTotal / sim_state::get_inst().numCpu));

    process_res(std::string("utilization: ") + std::to_string((global::utilTotal / (sim_state::get_inst().currTime /* sim_state::get_inst().numCpu*/)) * 100 ) + std::string("%"));
    process_res(std::string("throughput: ") + std::to_string(sim_state::get_inst().taskCt / sim_state::get_inst().currTime) + std::string("tasks/unit time"));
